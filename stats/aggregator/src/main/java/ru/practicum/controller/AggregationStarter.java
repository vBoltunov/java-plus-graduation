...
public class AggregationStarter {
        ...
        try {
            while (running.get()) {
                ...
                for (ConsumerRecord<Long, UserActionAvro> consumerRecord : records) {
                    List<EventSimilarityAvro> similarities = repository.updateEventSimilarity(consumerRecord.value());
                    for (EventSimilarityAvro similarity : similarities) {
                        sendEventSimilarity(similarity);
                    }
                    processedCount++;
                    updateOffsets(consumerRecord, processedCount);
                }
            }
    ...
    private void updateOffsets(ConsumerRecord<Long, UserActionAvro> consumerRecord, int count) {
        offsetMap.put(
